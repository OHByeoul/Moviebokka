...
public class ReviewController extends HttpServlet {
	...
	private ReviewRequestMapping reviewRequestMapping;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ReviewController() {
        super();
        reviewRequestMapping = new ReviewRequestMapping();
    }
	...
	private void requestMapper(HttpServletRequest request, HttpServletResponse response) {
		...
		if(route.length<Constants.THIRD) {
			reviewRequestMapping.dispatcherRoute(route[Constants.FIRST],request,response);
				getView = Route.getViewResolver(route[Constants.FIRST]);
		} else {
			reviewRequestMapping.dispatcherRoute(route[Constants.SECOND],request,response);
