...
public class CommentController extends HttpServlet {
	...
	private CommentRequestMapping commentRequestMapping;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CommentController() {
        super();
        commentRequestMapping = new CommentRequestMapping();
    }
	...
	private void requestMapper(HttpServletRequest request, HttpServletResponse response) {
		...
		if(route.length<Constants.THIRD) {
			commentRequestMapping.dispatcherRoute(route[Constants.FIRST],request,response);
				getView = Route.getViewResolver(route[Constants.FIRST]);
		} else {
			commentRequestMapping.dispatcherRoute(route[Constants.SECOND],request,response);
