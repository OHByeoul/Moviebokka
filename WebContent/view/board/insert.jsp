...
<c:if test="${(type == 2 or type == 3) and mem_id != 1}">
    <c:redirect url="boardList?type=1"></c:redirect>
</c:if>
<jsp:include page="../partial/header.jsp"/>
<title>
<c:if test="${brd_id == 0}">
<c:choose>
    <c:when test="${type == 1}">자유게시판 글작성</c:when>
    <c:when test="${type == 2}">공지사항 글작성</c:when>
    <c:when test="${type == 3}">FAQ 글작성</c:when>
</c:choose>
</c:if>
<c:if test="${brd_id != 0}">
<c:choose>
    <c:when test="${type == 1}">자유게시판 답글작성</c:when>
    <c:when test="${type == 2}">공지사항 답글작성</c:when>
    ...
    <div class="row">
        <div class="col-12">
            <div class="container mboard board-input-wrapper">
                <div class="row">
                    <c:if test="${brd_id == 0}">
                    <div class="col-sm-12 text-center board-title"><h3>글 작성</h3></div>
                    </c:if>
                    <c:if test="${brd_id != 0}">
                    <div class="col-sm-12 text-center board-title"><h3>답글 작성</h3></div>
                    </c:if>
                </div>
                <div class="row">
