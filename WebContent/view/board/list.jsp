...
<title>
<c:choose>
    <c:when test="${type == 1}">게시판 목록</c:when>
...
<div class="container">
    <div class="row">
        <div class="col-sm-12 text-center">
            <h3>
            <c:choose>
                <c:when test="${type == 1}">게시판</c:when>
                <c:when test="${type == 2}">공지</c:when>
                <c:when test="${type == 3}">FAQ</c:when>
            </c:choose>
            </h3>
        </div>
    </div>
            ...
            <div class="input-group">
                <div class="input-group-btn board-search">
                    <button type="button" class="btn btn-default dropdown-toggle" id="btn-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        ...
                        <c:choose>
                            <c:when test="${empty param.search}">선 택 </c:when>
                ...
                <span class="input-group-btn">
                    <button class="btn btn-default btn-board-search" type="button">
                        ...
                        <c:if test="${not empty search}"><span class="badge badge-search"> ${totCnt}</span></c:if>
                    </button>
                </span>
            </div>
        </div>
